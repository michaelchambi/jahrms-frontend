...
@Component({
  selector: 'app-office-details-view',
  templateUrl: './office-details-view.component.html',
  styleUrls: ['./office-details-view.component.css']
})
export class OfficeDetailsViewComponent {
  ...
  getWorkstation(id:any) {
    ...
    this.station.showWorkstation(this.data_required).subscribe(
      res => {
  ...
  userDetails(id: any) {
    ...
    this.users.showUser(id).subscribe(
      res => {
        ...
        this.getWorkstation(this.details.data.id);
      },
      err => {
        this.script.errorAlert(err.error.sw_message)
        if (err.error.token == 0) {
