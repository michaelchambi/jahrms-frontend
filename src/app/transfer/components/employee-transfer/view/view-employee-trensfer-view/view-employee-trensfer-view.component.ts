...
@Component({
  selector: 'app-view-employee-trensfer-view',
  templateUrl: './view-employee-trensfer-view.component.html',
  styleUrls: ['./view-employee-trensfer-view.component.css']
})
export class ViewEmployeeTrensferViewComponent {
  ...
  ngOnInit(): void {
    this.InitietedByEmployee();
    this.getUsersToTransfer();
    this.getTransfers()
  ...
  getTransfers() {
    ...
    this.transfer.getTransfers().subscribe(
      res => {
        this.userList = res.data;
...
getUsersToTransfer(){
  this.users.getAllUsers().subscribe(
    res =>{
      this.all_users_for_transfer= res.data
    },
    err =>{
      if (err.error.token == 0) {
        sessionStorage.setItem('current_url', this.route.url)
